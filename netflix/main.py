...
for k in range(len(K)):
    for i in range(len(seeds)):
        # Run kMeans for Project 4.2
        mixture, post = common.init(X, K[k], seeds[i]) #using for both methods
        mixtures_kMeans[i], posts_kMeans[i], costs_kMeans[i] = kmeans.run(X, mixture, post)

        # Run Naive EM for Project 4.4
        mixtures_EM[i], posts_EM[i], costs_EM[i] = naive_em.run(X, mixture, post)

    # Print lowest cost
    print("=============== Clusters:", k + 1, "======================")
    print("Lowest cost using kMeans is:", np.min(costs_kMeans)) # for Project 4.2
